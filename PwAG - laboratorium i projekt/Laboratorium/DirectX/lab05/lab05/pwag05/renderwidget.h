...
private:
	...
	void dxCreateShaderObjects(ID3DBlob *&vsBlob, ID3DBlob *&hsBlob, ID3DBlob *&dsBlob, ID3DBlob *&psBlob);			//we need the vs blob to be used for layout init
	...
	void dxConfigureHullShaderStage();
	void dxConfigureDomainShaderStage(bool sceneConstantsOutdated = true, bool objectConstantsOutdated = true, bool forceCBUpdate = false);
	...
	ID3D11Buffer *m_pConstantBuffer_scene, *m_pConstantBuffer_object, *m_pConstantBuffer_division;
	...
	ID3D11HullShader *m_pHullShader;				//compiled shader for HS lab5
	ID3D11DomainShader *m_pDomainShader;			//compiled shader for DS lab5
	...
	bool m_CameraDataDirty, m_ObjPosDataDirty, m_ForceConstBuffersUpdate;
