 ...
 * @author Frogi
 */
@WebServlet(name = "zmien_dzielnice", urlPatterns = {"/zmien_dzielnice"})
public class zmien_dzielnice extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (zalogowany == 'u') {
            try (PrintWriter out = response.getWriter()) {
                ...
                String nowa_dzielnica = request.getParameter("nowa_dzielnica");

                if (nowa_dzielnica.length() == 0) {
                    out.println("<h2 style=\"color: #BB1245;\">");
                    ...
                    out.println("Dzielnica zmieniona!");
                    out.println("</h2>");
                    out.println("<form method=POST action=Edycja_projektu>"); // Tutaj wracamy do edycji projektu o id IDP
                    out.println("<input name=\"IDP\" value=" + IDP + " style=\"display: none;\"/>"); // to już jest znane
                    out.println("<input type=submit value=\"Powrót\">");
